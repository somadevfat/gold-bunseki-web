...
export function AuthUI() {
  ...
  if (isPending) {
    return <div data-testid="auth-loading" className="text-sm text-slate-400 animate-pulse">読み込み中...</div>;
  }

  if (session && session.user) {
    return (
      <div data-testid="auth-user" className="flex items-center gap-2 p-1 pr-3 bg-white border border-slate-200 rounded-full shadow-sm hover:shadow-md transition-shadow duration-200">
        {session.user.image ? (
          <img
        ...
        <div className="flex flex-col">
          <span className="text-[11px] leading-tight text-slate-500 font-medium px-1">ログイン中</span>

...
このプロジェクトでは、時間がかかってもユーザー自身が実装を行うことを優先します。
AI はコードを直接変更・実装してはならず、設計整理、実装手順の分解、参考コードの説明、レビュー、デバッグ支援に徹すること。
コミットは **@commitlint/config-conventional に従い、本文・件名とも英語のみ**（`commitlint.config.js` の `english-only` ルールで非 ASCII を拒否）。
...
- **AI はサポート専任**: ユーザーから明示的な依頼がない限り、AI はコード編集・ファイル作成・依存追加・コミットを直接実行しない。必要な場合は、ユーザーが自分で進められるように手順、差分方針、確認観点を提示すること。
